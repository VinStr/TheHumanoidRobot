...
def TTS(speechTxt,filename):
    ...
    v.save('speech/temp.wav')
    cmd = 'ffmpeg -i '+ os.path.abspath('speech/temp.wav')+' -af adelay=100 '+ os.path.abspath(filename)
    os.system(cmd)
...
def getPhonemesTiming(speechTxt,csvFilename):
    ...
    indexer = sai(mode = "cmu", src_dir=os.path.abspath("speech"))
...
while True:
    ...
    audioFilename = getAudioFilename(txtTmp,True)
    csvFilename = getCsvFilename(txtTmp,True)
